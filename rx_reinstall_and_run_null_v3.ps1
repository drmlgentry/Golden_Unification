# ============================================================
# rx_reinstall_and_run_null_v3.ps1 (FIXED)
# ============================================================
...
if (-not (Test-Path -LiteralPath $code))   { New-Item -ItemType Directory -Path $code   | Out-Null }
...
Write-Host ("Wrote: " + $pyPath) -ForegroundColor Green

Write-Host ("Running: python -u " + $pyPath) -ForegroundColor Cyan
python -u $pyPath --N 2000 --seed 12345 --out $outTex
if ($LASTEXITCODE -ne 0) { throw ("Python failed with exit code: " + $LASTEXITCODE) }

$fi = Get-Item -LiteralPath $outTex
Write-Host ("OK: " + $fi.FullName) -ForegroundColor Green
Write-Host ("Bytes: " + $fi.Length) -ForegroundColor Green
